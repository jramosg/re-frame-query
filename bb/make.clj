(ns make
  ...
  `bb tag 0.11.0` promotes the changelog's Unreleased section into a dated
...
(def ^:private repo-url "https://github.com/shipclojure/re-frame-query")
...
(defn- previous-release
  ...
  changelog entry, so linking to v0.11.0-alpha2 would show a diff that
...
(defn- bump-readme!
  ...
  [version]
  (doseq [re [#"(com\.shipclojure/re-frame-query \{:mvn/version \")[^\"]*(\")"
              #"(\[com\.shipclojure/re-frame-query \")[^\"]*(\")"]]
...
(defn- ensure-releasable! [version]
  ...
  ;; 0.11.0-alpha1 — dot-separated alphanumeric identifiers.
...
(defn tag
  "Prepare and publish a release. Usage: bb tag 0.11.0"
  [& [version]]
  (let [version (some-> version str/trim not-empty)]
    (when-not version
      (fail! "Usage: bb tag <version>   e.g. bb tag 0.11.0"))
...
(defn changelog-entry
  ...
  Defaults to the latest tag. Usage: bb changelog-entry [0.10.0]"
  [& [version]]
